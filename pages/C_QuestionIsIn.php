...
*/
  ...
  class QuestionIsIn{
    ...
    public function insert(){
      ...
      $sql = "INSERT INTO question_is_in
      ...
      if($isQuestionSetIdExist){
        if($isQuestionIdExist){
          // questionSetId and questionId exist; attempt to insert
          try{
          ...
          $response = array();
          $response["message"] = "ERROR INSERTING into question_is_in table: question_id ".$this->question_id." does not exist in question table";
          $response["success"] = 0;
          return json_encode($response);
        }
      }else{
        ...
        $response = array();
        $response["message"] = "ERROR INSERTING into question_is_in table: question_set_id ".$this->question_set_id." does not exist in question_set table";
        $response["success"] = 0;
        return json_encode($response);
      }
    }
...
//      include('Connection.php');
      ...
      $pdo = null;
      $response = array();
      $response["message"] = "Success SELECTING from question_is_in";
      $response["success"] = 1;
